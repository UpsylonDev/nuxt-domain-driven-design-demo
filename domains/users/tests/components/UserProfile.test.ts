...
describe('UserProfile', () => {
  ...
  it('renders all user fields in list format', () => {
    ...
    expect(listItems[0].text()).toBe('Name: John Doe')
    expect(listItems[1].text()).toBe('Email: john.doe@example.com')
    expect(listItems[2].text()).toBe('Username: johndoe')
    expect(listItems[3].text()).toBe('Initials: JD')
  })
})
