...
class ProjectAnalyzer {
    ...
    if (fs.existsSync(typesPath)) {
      ...
      if (typesContent.includes('Entity') || typesContent.includes('ValueObject')) {
        ...
        this.improvements.push(
          `${domain}: Consider implementing explicit Entity and ValueObject patterns`
        )
      }
    }
  ...
  async analyzeCodeQuality() {
    ...
    const eslintConfig = ['.eslintrc.js', '.eslintrc.json', 'eslint.config.js'].some(config =>
    ...
    const prettierConfig = ['.prettierrc', '.prettierrc.json', 'prettier.config.js'].some(config =>
      fs.existsSync(path.join(this.projectRoot, config))
    )
  ...
  async analyzeConfiguration() {
    ...
    const hasEnvConfig = ['.env.example', '.env.local', 'nuxt.config.ts'].some(config =>
      fs.existsSync(path.join(this.projectRoot, config))
    )
    ...
    const hasDocker = ['Dockerfile', 'docker-compose.yml'].some(config =>
      fs.existsSync(path.join(this.projectRoot, config))
    )
    ...
    const hasCI = ['.github/workflows', '.gitlab-ci.yml', 'azure-pipelines.yml'].some(config =>
      fs.existsSync(path.join(this.projectRoot, config))
    )
    ...
    try {
      ...
      for (const file of files) {
        if (file.isFile() && file.name.endsWith('.ts')) {
          ...
          const found = await this.searchInDirectory(path.join(dirPath, file.name), pattern)
  ...
  async checkForApiDocumentation() {
    ...
    return docsPatterns.some(pattern => fs.existsSync(path.join(this.projectRoot, pattern)))
  }
  ...
  printGrade(percentage) {
    ...
    if (percentage >= 90) {
      ...
      grade = 'A'
      emoji = '⭐'
      description = 'Excellent - Minor improvements needed'
    } else if (percentage >= 70) {
      grade = 'B'
      emoji = '👍'
      description = 'Good - Some enhancements required'
      ...
      grade = 'D'
      emoji = '🔧'
      description = 'Needs Work - Major restructuring required'
    }
